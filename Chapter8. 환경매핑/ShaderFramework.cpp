...
void RenderScene()
{
	...
	D3DXVECTOR3 vEyePt(gWorldCameraPosition.x,gWorldCameraPosition.y, gWorldCameraPosition.z);					//카메라위치
...
bool LoadAssets()
{
	...
	D3DXCreateCubeTextureFromFile(gpD3DDevice, "Snow_ENV.dds", &gpSnowENV);
								//gpD3DDevice : D3D장치
								//Snow_ENV.dds : 로딩할 입방체 텍스처의 파일명
								//&gpSnowENV : 로딩된 텍스처를 저장할 텍스처 포인터
	if (!gpSnowENV)
	{
		return false;
	}
